...
// --- Configuration ---
const int N_SAMPLES = 128;               // FFT sample size (must be power of 2)
...
bool collectSamples(int dataBuffer[], int numSamples, float samplingFrequencyHz, int analogPin) 
{
    ...
    for (int i = 0; i < numSamples; i++) 
    {
        ...
        // (analogRead gives 0-1023, so subtracting 512 centers it)
...
void loop()
{
    ...
    */
    ...
    for (int i = 0; i < 5; i++) {
        Serial.print("Peak "); Serial.print(i); Serial.print(": Freq=");
    ...
    // Wait 2 seconds before collecting the next batch
    delay(100);
}
